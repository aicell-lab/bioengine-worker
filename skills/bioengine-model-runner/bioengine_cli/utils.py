...
async def connect_model_runner(server_url: str, token: Optional[str] = None):
    ...
    # When multiple replicas are running, get_service("bioimage-io/model-runner") raises
    # "Multiple services found". Fall back to listing services and picking the first match.
    try:
        ...
        async with httpx.AsyncClient(follow_redirects=True) as client:
            r = await client.get(f"{server_url}/bioimage-io/services", timeout=15)
            r.raise_for_status()
            services = r.json()
        runner_ids = [
            f"bioimage-io/{s['id']}"
            for s in services
            if s.get("type") == "bioengine-apps" and "model-runner" in s.get("id", "")
        ]
        if not runner_ids:
            raise RuntimeError("No model-runner service found in bioimage-io workspace") from exc
        service = await server.get_service(runner_ids[0])
    return service
