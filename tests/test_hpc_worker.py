"""Tests for BioEngineWorker class."""
...
@pytest.fixture
def bioengine_worker(temp_config_file):
    """Create an BioEngineWorker instance with a temporary config.
    ...
    worker = BioEngineWorker(config_path=temp_config_file)
...
def test_bioengine_worker_init(temp_config_file):
    """Test initialization of BioEngineWorker."""
    worker = BioEngineWorker(config_path=temp_config_file)
...
def test_bioengine_worker_create_config():
...
def test_bioengine_worker_init_missing_params():
...
def test_format_time(bioengine_worker):
    """Test formatting timestamp with duration."""
    # Test with current timestamp minus 1 hour
    timestamp = int(time.time()) - 3600
    
    time_info = bioengine_worker.format_time(timestamp)
...
def test_format_time_complex_duration(bioengine_worker):
...
def test_get_worker_status(bioengine_worker, temp_config_file):
    ...
    with patch('os.path.exists', side_effect=mock_path_exists):
        with patch.object(bioengine_worker.ray_manager, 'check_cluster') as mock_check_cluster:
            ...
            status = bioengine_worker.get_worker_status()
...
def test_cleanup_sync(bioengine_worker):
    """Test cleanup method properly shuts down resources."""
    # Mock the ray_manager methods
    bioengine_worker.ray_manager.shutdown_cluster = MagicMock(return_value={"success": True})
    bioengine_worker.ray_manager.cancel_worker_jobs = MagicMock(return_value={
        "success": True,
        "cancelled_jobs": 2
    })
    ...
    loop = asyncio.get_event_loop()
    result = loop.run_until_complete(bioengine_worker.cleanup())
    
    # Verify ray manager methods were called
    assert bioengine_worker.ray_manager.shutdown_cluster.called
    assert bioengine_worker.ray_manager.cancel_worker_jobs.called
...
# @pytest.mark.asyncio
# async def test_cleanup(bioengine_worker):
#    ...
