...
class LocalBioimageioPackage:
    ...
    async def get_download_time(self) -> Optional[float]:
        """Get the download timestamp of this model package."""
        download_file = self.package_path / ".download_time"
        if not download_file.exists():
            return None

        try:
            download_content = await asyncio.to_thread(download_file.read_text)
            return float(download_content.strip())
        except (ValueError, FileNotFoundError, OSError, IOError) as e:
            print(f"⚠️ [{self.replica_id}] Failed to read download time: {e}")
            return None
    ...
    async def validate(self) -> Path:
        ...
        try:
            ...
            model_description = await asyncio.to_thread(
                load_model_description, model_source
            )
...
class ModelDownloader:
    ...
    async def async_init(self) -> None:
        ...
        for package_dir in items:
            try:
                ...
                # Create download time file if it doesn't exist
                download_file = package_dir / ".download_time"
                if not download_file.exists():
                    try:
                        await asyncio.to_thread(
                            download_file.write_text, str(time.time())
                        )
                    except (OSError, IOError) as e:
                        print(
                            f"⚠️ [{self.replica_id}] Failed to create download time file for {cache_key}: {e}"
                        )
    ...
    async def test_deployment(
        self, model_id: str = "ambitious-ant", test_skip_cache: bool = False
    ...
    # === Internal Methods - downloading and caching models ===
    ...
    async def _get_cached_models_info(self) -> List[Dict[str, Union[str, float, bool]]]:
        ...
        for item in items:
            try:
                ...
                download_file = item / ".download_time"
                ...
                if download_file.exists():
                    try:
                        download_content = await asyncio.to_thread(
                            download_file.read_text
                        )
                        download_time = float(download_content.strip())
                    except (ValueError, FileNotFoundError, OSError, IOError):
                        download_time = 0
    ...
    async def _download_model_from_url(self, model_id: str, package_path: Path) -> None:
        ...
        download_url = f"{self.server_url}/{self.workspace}/artifacts/{model_id}/create-zip-file?stage=true"
    ...
    async def _download_model(self, cache_key: str) -> Path:
        ...
        try:
            ...
            # Create download time and last access files
            current_time = time.time()
            download_file = package_dir / ".download_time"
            access_file = package_dir / ".last_access"

            try:
                await asyncio.to_thread(download_file.write_text, str(current_time))
            except (OSError, IOError) as e:
                print(
                    f"⚠️ [{self.replica_id}] Failed to create download time file for new model: {e}"
                )
    ...
    async def get_model_rdf(
        ...
        """
        print(f"📋 [{self.replica_id}] Downloading RDF for model '{model_id}'")
        ...
        async with httpx.AsyncClient(
            ...
            model_rdf = yaml.safe_load(response.text)
    ...
    async def test(
        ...
        try:
            ...
            async with local_package:
                ...
                test_result = await self.model_runner.test.remote(
    ...
    async def infer(
        ...
        try:
            ...
            async with local_package:
                model_source = str(local_package.model_source)
                download_time = await local_package.get_download_time()
                print(
                    f"📍 [{self.replica_id}] Model source for '{model_id}': {model_source} (downloaded: {download_time})"
                )

                result = await self.model_runner.predict.remote(
                    model_source=model_source,
                    inputs=inputs,
...
if __name__ == "__main__":
    ...
    try:
        model_downloader = ModelDownloader.func_or_class(
            model_runner=MockHandle(),
            max_models=2,
        )
        asyncio.run(model_downloader.async_init())

        asyncio.run(model_downloader.test_deployment(test_skip_cache=True))
    finally:
