...
A prebuilt Docker image is available under `ghcr.io/aicell-lab/bioengine-worker:0.1.9` (change the version if needed, see [all the versions](https://github.com/orgs/aicell-lab/packages/container/package/bioengine-worker)).
...
```bash
docker run --rm ghcr.io/aicell-lab/bioengine-worker:0.1.9 python -m bioengine_worker --help
```
