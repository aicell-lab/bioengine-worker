...
class HpcWorker:
    ...
    def __init__(
        ...
        if config_path:
            ...
            if not os.path.exists(config_path) or os.path.getsize(config_path) == 0:
                ...
                if num_gpu is not None and dataset_paths is not None and trusted_models is not None:
    ...
    def format_time(self, last_deployed_time_s: int, tz: timezone = timezone.utc) -> Dict:
    ...
    async def register(self) -> Dict:
        ...
        try:
            ...
            service_url = f"{self.server_url}/{self.server.config.workspace}/services/{sid}"
            
            # Log service endpoints
            self.logger.info(f"Test the HPC worker service here: {service_url}/get_worker_status")
            self.logger.info(f"Start Ray cluster with: {service_url}/start_ray_cluster")
            self.logger.info(f"Shutdown Ray cluster with: {service_url}/shutdown_ray_cluster")
            self.logger.info(f"Submit a Ray worker job with: {service_url}/submit_ray_worker_job")
            self.logger.info(f"Get Ray worker jobs status with: {service_url}/get_ray_worker_jobs")
            self.logger.info(f"Cancel Ray worker jobs with: {service_url}/cancel_ray_worker_jobs")
