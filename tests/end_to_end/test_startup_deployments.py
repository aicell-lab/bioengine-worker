...
async def test_worker_initialization_and_readiness(
    ...
    await asyncio.wait_for(
        bioengine_worker.is_ready.wait(), 
        timeout=worker_ready_timeout
    )
    
    # Worker should be ready
    assert bioengine_worker.is_ready.is_set(), "Worker should be ready after initialization"
...
async def test_worker_service_registration(
    ...
    # Worker is already started by the fixture, just wait for readiness
    await asyncio.wait_for(bioengine_worker.is_ready.wait(), timeout=worker_ready_timeout)
...
async def test_startup_applications_deployment(
    ...
    for app_config in startup_applications:
        ...
        assert app_id in bioengine_apps, f"Application {app_id} not found in worker status"
        
        app_status = bioengine_apps[app_id]
        assert app_status["status"] == "RUNNING", f"Application {app_id} is not running: {app_status['status']}"
        
        # Verify service IDs are present
        service_ids = app_status.get("service_ids", {})
        assert "webrtc_service_id" in service_ids, f"WebRTC service ID missing for {app_id}"
        assert "websocket_service_id" in service_ids, f"WebSocket service ID missing for {app_id}"
        
        # Verify available methods
        available_methods = app_status.get("available_methods", [])
        assert len(available_methods) > 0, f"No available methods for application {app_id}"
        assert "ping" in available_methods, f"'ping' method not found for application {app_id}"
...
async def test_application_websocket_connectivity(
    ...
    for app_config in startup_applications:
        ...
        assert "/" in websocket_service_id, f"Invalid WebSocket service ID format: {websocket_service_id}"
        ...
        assert service_name == app_id, f"Service name mismatch: expected {app_id}, got {service_name}"
        
        # Connect to application service
        app_service = await hypha_client.get_service(websocket_service_id)
        assert app_service, f"Could not connect to WebSocket service {websocket_service_id}"
...
async def test_application_webrtc_connectivity(
    ...
    for app_config in startup_applications:
        ...
        assert webrtc_service_id.endswith("rtc"), f"Invalid WebRTC service ID: {webrtc_service_id}"
